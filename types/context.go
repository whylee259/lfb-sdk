...
func (c Context) BlockHeader() ostproto.Header {
	var msg = proto.Clone(&c.header).(*ostproto.Header)
	return *msg
}
...
func NewContext(ms MultiStore, header ostproto.Header, isCheckTx bool, logger log.Logger) Context {
...
func (c Context) WithBlockHeader(header ostproto.Header) Context {
	// https://github.com/gogo/protobuf/issues/519
	header.Time = header.Time.UTC()
	c.header = header
	return c
...
func (c Context) KVStore(key StoreKey) stypes.KVStore {
